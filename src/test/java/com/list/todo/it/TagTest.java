...
public class TagTest {
    ...
    private final static String TODOLIST_NAME = "todoList";
    ...
    public void getMyTagsWithTodoListId_OnExistentTagsIds_ReturnsAListOfTagTaskKeys() throws Exception {
        ...
        when(tagTaskKeyServiceMock.getMyTaggedTask(any(UserPrincipal.class), any(Pageable.class), eq(tagsIds), eq(TODOLIST_NAME)))
                .thenReturn(createListOfTaggedTask(CURRENT_USER_ID));

        //act
        MvcResult result = this.mockMvc.perform(get("/api/tags/myTagTaskKeys?tagId=" + TAG_ID + "&searchQuery=" + TODOLIST_NAME))
                .andDo(print())
                .andExpect(status().isOk())
                .andReturn();
        ...
        verify(tagTaskKeyServiceMock, times(1)).getMyTaggedTask(any(UserPrincipal.class), any(Pageable.class), eq(tagsIds), eq(TODOLIST_NAME));
    }
