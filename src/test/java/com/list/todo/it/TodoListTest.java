...
public class TodoListTest {
    ...
    public void getTodoLists_getExistingTodoLists_OkStatus() throws Exception {
        ...
        when(todoListService.getTodoListsByUser(any(UserPrincipal.class), eq(TodoListStatus.ACTIVE), any(Pageable.class),
                eq(new ArrayList<>()), eq(TODO_LIST_NAME))).thenReturn(todoLists);

        // act
        MvcResult result = this.mockMvc.perform(get("/api/todolists?status={status}&searchQuery={todoListName}&tagId=", TodoListStatus.ACTIVE, TODO_LIST_NAME)
        ...
        // assert
        verify(todoListService, times(1)).getTodoListsByUser(any(UserPrincipal.class),
                eq(TodoListStatus.ACTIVE), any(Pageable.class), eq(new ArrayList<>()), eq(TODO_LIST_NAME));
